...
public class MainController {
	...
	public ModelAndView selectMainList(CommandMap commandMap, HttpServletRequest request)  // 메인 json
		 ...
		 * goodsService.newGoodsList(commandMap.getMap()); newList.get(0); if
		 * (newList.size() != 0) { newList.get(0).get(""); }
		 * 
		 * mv.addObject("NewList", newList); if (newList.size() > 0) {
		 * mv.addObject("TOTAL", newList.get(0).get("TOTAL_COUNT")); } else {
		 * mv.addObject("TOTAL", 0); }
		 * 
		 * List<Map<String,Object>> bestList =
		 * goodsService.bestGoodsList(commandMap.getMap()); bestList.get(0); if
		 * (bestList.size() != 0) { bestList.get(0).get(""); }
		 * 
		 * mv.addObject("BestList", bestList); if (bestList.size() > 0) {
		 * mv.addObject("TOTAL1", bestList.get(0).get("TOTAL_COUNT")); } else {
