-- =====================================================================
-- 티켓팅 AI 보안관제 시스템 — Oracle 21c DDL (통합 최종 수정본)
-- 기반         : 사용자 제공 ERD export DDL
-- 수정사항     :
--   ① 모든 PK에 GENERATED AS IDENTITY 추가 (DBeaver 완벽 동기화)
...
--   ⑦ attachments - qna_posts 간 상호 맵핑용 자동 연동 트리거 내장
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- (선택) 기존 테이블 및 트리거 제거 — 처음 실행 시 주석 해제 후 사용
-- ---------------------------------------------------------------------
-- DROP TRIGGER chatbot_logs_trg; -- 필요 시 주석 해제
-- DROP TRIGGER trg_attach_post_id_fill;
...
-- =====================================================================
-- 11. qna_posts  (Q&A 게시글) - 생성 순서 변경 (attachments가 참조하기 위함)
-- =====================================================================
...
-- =====================================================================
-- 12. attachments  (첨부파일) - 튜닝 및 DBeaver 규격 반영본
-- =====================================================================
CREATE TABLE attachments (
    attachment_id   NUMBER(19,0)    GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    post_id         NUMBER(19,0),   -- posts 혹은 qna_posts의 ID를 공유하여 관리
    ...
    CONSTRAINT fk_attachments_post  FOREIGN KEY (post_id)
        REFERENCES posts (post_id),
    -- QNA 테이블 연동을 위한 외래키(FK) 명시적 추가
    CONSTRAINT fk_attachments_qna   FOREIGN KEY (post_id)
        REFERENCES qna_posts (qna_id)
);
...
CREATE INDEX idx_qna_posts_member      ON qna_posts (member_id);   -- QNA 회원 인덱스 보강
CREATE INDEX idx_attachments_post      ON attachments (post_id);   -- 첨부파일 FK 인덱스 보강
...
-- =====================================================================
-- 연동 트리거 (오라클 PL/SQL 컴파일 블록)
-- =====================================================================
-- 자바 애플리케이션 업로드 세션에서 QNA_ID 맵핑이 누락되어 인서트될 경우의 자동 방어용 트리거
CREATE OR REPLACE TRIGGER trg_attach_post_id_fill
BEFORE INSERT ON attachments
FOR EACH ROW
BEGIN
    IF :NEW.post_id IS NULL OR :NEW.post_id = 0 THEN
        SELECT NVL(MAX(qna_id), 0) INTO :NEW.post_id FROM qna_posts;
    END IF;
END;
/
