...
<body>
	...
	<div class="tk-page-head">
		<div class="tk-page-head-inner">
			...
			<div class="tk-steps">
				<span class="tk-step"><span class="num">1</span>공연 선택</span> <span
					class="tk-step-sep">›</span> <span class="tk-step active"><span
					class="num">2</span>좌석 선택</span> <span class="tk-step-sep">›</span> <span
					class="tk-step"><span class="num">3</span>결제</span> <span
					class="tk-step-sep">›</span> <span class="tk-step"><span
					class="num">4</span>완료</span>
			</div>
		</div>
	</div>
	...
	<div class="ss-layout">
		...
		<main class="ss-center">
			...
			<div class="legend">
				<div class="legend-item">
					<div class="legend-box" style="background: #e8001c"></div>
					예매가능
				</div>
				<div class="legend-item">
					<div class="legend-box" style="background: #bdbdbd"></div>
					예매불가
				</div>
				<div class="legend-item">
					<div class="legend-box" style="background: #f5c842"></div>
					선택됨
				</div>
			</div>
		</main>
		...
		<aside class="ss-side">
			...
			<div class="zone-tabs" id="zoneTabs">
				<p class="tk-text-muted" style="grid-column: 1/-1;">구역 정보를 불러오는
					중...</p>
			</div>
			...
			<div class="ss-actions">
				<button onclick="goToBooking()"
					class="tk-btn tk-btn-primary tk-btn-lg">🎫 예매하기</button>
			</div>
		</aside>
	...
	<script>
		...
		var selectedSeats = JSON.parse(sessionStorage.getItem('selectedSeats_'
				+ scheduleId)
				|| '[]');
		...
		function renderZoneTabs() {
			...
			for (var i = 0; i < ZONE_COUNT; i++) {
				...
				var badge = mineInZone > 0 ? '<span class="badge">'
						+ mineInZone + '</span>' : '';
		...
		function loadZoneSeats(zone) {
			document.getElementById('seatMap').innerHTML = '<p class="tk-text-muted">'
					+ zone + '구역 좌석을 불러오는 중...</p>';

			var xhr = new XMLHttpRequest();
			xhr.open('GET', '/seat/zone.do?scheduleId=' + scheduleId + '&zone='
					+ zone, true);
			xhr.onload = function() {
				if (xhr.status === 200) {
					...
					document.getElementById('seatMap').innerHTML = '<p class="tk-text-muted">좌석을 불러올 수 없습니다.</p>';
				}
			};
			xhr.onerror = function() {
				document.getElementById('seatMap').innerHTML = '<p class="tk-text-muted">네트워크 오류가 발생했습니다.</p>';
			};
			xhr.send();
		}
		...
		function renderSeats(seats) {
			...
			seats.forEach(function(seat) {
				...
				html += ' onclick="selectSeat(' + seat.seatId + ', \''
						+ seat.status + '\', ' + seat.seatRow + ', '
						+ seat.seatCol + ')"';
				html += ' title="' + seat.seatRow + '열 ' + seat.seatCol
						+ '번 / ' + seat.price + '원">';
				html += '</div>';
			});
			if (currentRow !== null)
		...
		function updateInfo() {
			if (selectedSeats.length === 0) {
				document.getElementById('info').innerHTML = '<div class="label">선택한 좌석</div>좌석을 선택해 주세요';
				return;
			}
			...
			msg += '<span style="font-size:13px; color:#555;">' + seatLabels
					+ '</span>';
	...
	<script>
		...
		(function() {
			...
			document.addEventListener('click', function(e) {
				...
				if (target.closest
						&& (target.closest('.seat-cell')
								|| target.closest('.seat-btn') || target
			...
			function injectPageLoadTs(formEl) {
				/* 이미 있으면 값만 갱신 */
				var existing = formEl
						.querySelector('input[name="seat_page_load_ts"]');
				if (existing) {
				...
				$.ajax({
					...
					success : function(data) {
						if (data && data.avg_ms < 100) {
							console.warn('[SECURITY] avg_click=' + data.avg_ms
									+ 'ms — 비정상');
						}
					}
				});
			...
			$(document).on(
					'click',
					'#btn-booking, .btn-booking, [data-action="booking"]',
					function() {
						/* 클릭 간격 전송 */
						sendClickIntervals();
			...
			window.addEventListener('beforeunload', function() {
				if (clickTimestamps.length < 2)
					return;
				var body = 'timestamps='
						+ encodeURIComponent(clickTimestamps.join(','))
						+ '&concertId=' + encodeURIComponent(SCHEDULE_ID);
				navigator.sendBeacon('/log/clickInterval.do', new Blob(
						[ body ], {
